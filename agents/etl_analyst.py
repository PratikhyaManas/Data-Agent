...
decide which tool(s) to call, execute them, and report back. Once the
loop finishes (no more tool calls):
...
Both loops are capped (MAX_JUDGE_RETRIES / MAX_DQ_RETRIES) so a
stubbornly bad source can't loop forever; it surfaces a caveat instead.
"""
import re
...
def check_data_quality(state: ETLAgentSchema) -> ETLAgentSchema:
    ...
    if report["severity"] == "critical" and state.dq_retries < MAX_DQ_RETRIES:
        state.dq_retries += 1
...
def build_etl_agent():
    ...
    graph.add_conditional_edges("agent", should_continue, {"tools": "tools", "data_quality": "data_quality"})
